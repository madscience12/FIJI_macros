...
// GUI 
  ...
  Dialog.addNumber("Square Dimensions (pixels):", 50); //select size of square to crop around cells
  Dialog.addString("Ag end", "C2.tif"); //tell it what channel your antigen and signaling molecules are in 
  Dialog.addString("Sig end", "C0.tif");
  Dialog.addString("Rolling Ball (background subtraction)", "10"); //tell it what ball size to use for background subtraction 
...
inputDir = getDirectory("choose the input directory"); //get input directory
listdir = getFileList( inputDir ); 

print("Crop box dimensions (pixels):" + SquareSize);//make a file in the input directory with all the parameters that the user selected at the start
print("Antigen channel ID:" + AgEnd);
print("Signalling channel ID:" + SigEnd);
print("Rolling ball radius (pixels):" +Ball);
...
out_dir= path + "/output/"; //make some output directories to store everything 
...
function saverAg(){ //opens all the croped Ag cells into a stack and runs the analysis on them
	...
	run("Subtract Background...", "rolling=&Ball stack"); //rolling ball radius set to 2
	...
	run("Analyze Particles...", "size=0.10-Infinity display exclude add stack");  //size is 0.1-infinity uM
